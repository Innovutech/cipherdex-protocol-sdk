...
export const PUBLIC_LIQUIDITY_ROUTED_TOPIC =
  "0xf57701d662488466310aef8303deec83f9fbfa81b6327d4577152fbc12634d4b" as const;
export const NATIVE_LIQUIDITY_ADDED_TOPIC =
  "0x6241df93d44bd96caf1d9efe1ba8da46b54ba15df7beea93efec7c360f756e0e" as const;
...
function assertLiquiditySide(side: LiquiditySide): void {
...
export function liquiditySideToContractBoolean(side: LiquiditySide): boolean {
...
function assertPositiveUint256(value: bigint, label: string): void {
 ...
 */
export function previewPublicProportionalLiquidity(input: Readonly<{
  reserve0: bigint;
  reserve1: bigint;
  ...
  const specifiedReserve = input.specifiedSide === "token0"
    ? input.reserve0
    : input.reserve1;
  const expectedLpShares =
    input.specifiedAmount * input.totalLpShares / specifiedReserve;
  ...
  const acceptedAmount0 = ceilDiv(
    expectedLpShares * input.reserve0,
    input.totalLpShares,
  );
  const acceptedAmount1 = ceilDiv(
    expectedLpShares * input.reserve1,
    input.totalLpShares,
  );
  if (
    acceptedAmount0 === 0n ||
...
function snapshotLogs(receipt: LiquidityReceiptEvidence): readonly LiquidityLogEvidence[] {
  ...
  const logs: LiquidityLogEvidence[] = [];
...
function uniqueEventLog(
  logs: readonly LiquidityLogEvidence[],
  emitter: string,
  topic: string,
  label: string,
): LiquidityLogEvidence {
...
function topicBoolean(topic: string | undefined): boolean | undefined {
  if (!topic || !/^0x0{63}[01]$/.test(topic)) return undefined;
  return topic.endsWith("1");
}

function dataWords(data: string, expectedWords: number): readonly string[] | undefined {
  if (!new RegExp(`^0x[0-9a-fA-F]{${expectedWords * 64}}$`).test(data)) return undefined;
  const words: string[] = [];
...
function wordUint256(word: string | undefined): bigint | undefined {
  if (!word || !/^[0-9a-fA-F]{64}$/.test(word)) return undefined;
  return BigInt(`0x${word}`);
}

function assertReceiptTransaction(
  receipt: LiquidityReceiptEvidence,
  transactionHash: string,
): readonly LiquidityLogEvidence[] {
...
function decodePublicLiquidityLog(log: LiquidityLogEvidence): Readonly<{
  provider: string;
  pool: string;
  poolCreated: boolean;
  amount0: bigint;
...
export function parsePublicLiquidityRoutedResult(
  ...
  const decoded = decodePublicLiquidityLog(uniqueEventLog(
    logs,
    expectation.liquidityRouter,
    PUBLIC_LIQUIDITY_ROUTED_TOPIC,
    "PublicLiquidityRouted",
  ));
  if (
    !sameAddress(decoded.provider, expectation.provider) ||
 ...
 */
export function parseNativeLiquidityAddedResult(
  expectation: Readonly<{
    transactionHash: string;
    nativeRouter: string;
    liquidityRouter: string;
  ...
  ] as const) assertAddress(value, label);
  ...
  const logs = assertReceiptTransaction(receipt, expectation.transactionHash);
  const nativeLog = uniqueEventLog(
    logs,
    expectation.nativeRouter,
  ...
  const routed = decodePublicLiquidityLog(uniqueEventLog(
    logs,
    expectation.liquidityRouter,
    PUBLIC_LIQUIDITY_ROUTED_TOPIC,
    "nested PublicLiquidityRouted",
  ));
  const wrappedIsToken0 =
    expectation.wrappedNative.toLowerCase() < expectation.pairedToken.toLowerCase();
  ...
  if (
    !sameAddress(routed.provider, expectation.recipient) ||
 ...
 */
export function parseConfidentialAddLiquidityQuoteResult(
  expectation: Readonly<{
    transactionHash: string;
    pool: string;
    caller: string;
  ...
  receipt: LiquidityReceiptEvidence,
): ConfidentialAddLiquidityQuoteResult {
  ...
  const logs = assertReceiptTransaction(receipt, expectation.transactionHash);
  const log = uniqueEventLog(
    logs,
    expectation.pool,
    CONFIDENTIAL_LIQUIDITY_QUOTE_RESULT_TOPIC,
    "ConfidentialLiquidityQuoteResult",
  );
  ...
  const acceptedAmount0 = expectation.specifiedSide === "token0"
    ? acceptedSpecified as LiquidityCiphertext256
    : counterpart as LiquidityCiphertext256;
  ...
  return Object.freeze({
    ...
    acceptedAmount0,
    acceptedAmount1,
    expectedLpShares: expectedLpShares as LiquidityCiphertext256,
  });
}
