...
import {ProtocolReadAdapter,ProtocolCodec,Address,Hex,address,hex,hash,uint,bool,field,optional,array,tuple,equal,fail,block,stable,ZERO_ADDRESS} from './protocolData.js';
import {verifyProtocolBundle,requireBundle,authenticateCandidates,VerifiedBundle,VerifiedPool} from './poolIdentity.js';
...
export interface CotiDecryptionAdapter {decryptResult(input:Readonly<{chainId:bigint;caller:Address;ciphertext:CallerCiphertext}>):Promise<unknown>;}
export type VerifiedPublicResult=Readonly<{transactionHash:Hex;bundle:VerifiedBundle;selectedPool:VerifiedPool;amountOut:bigint}>;
export type VerifiedConfidentialResult=Readonly<{transactionHash:Hex;bundle:VerifiedBundle;selectedPool:VerifiedPool;caller:Address;ciphertext:CallerCiphertext}>;
const privateResults=new WeakSet<object>();
export async function parseProtocolReceipt(call:ProtocolCall,transactionHash:Hex,adapter:ProtocolReceiptAdapter,codec:ProtocolCodec):Promise<VerifiedPublicResult|VerifiedConfidentialResult>{
 const c=context(call);if(c.kind==='public-quote'||c.kind==='position')fail('Use the matching result parser');
 ...
 const topic=(PROTOCOL_EVENT_TOPICS[label] as Readonly<Record<string,string>>)[eventName];
 const matches=logs.filter(l=>l.address===call.to&&l.topics[0]===topic);
 if(matches.length!==1)fail('Missing or ambiguous routed result');
 const log=matches[0];if(log.topics.length!==4)fail('Malformed indexed event');
 ...
 if(c.kind==='confidential-quote'){
  equal(hash(decoded[1]),c.requestId,'request ID');selected=address(decoded[2]);
 }else{selected=address(decoded[1]);equal(address(decoded[2]),c.recipient,'event recipient');}
 ...
 if(privateResult){
  ...
  privateResults.add(result);return result;
 }
 ...
 const amountOut=uint(decoded[6],256,true);if(amountOut<c.minimumOut!)fail('Event output below minimum');
 return Object.freeze({transactionHash:txHash,bundle:current,selectedPool:pool,amountOut});
}
export async function verifyReceiptTransaction(call:ProtocolCall,transactionHash:Hex,adapter:ProtocolReceiptAdapter){
 const c=context(call),result=await verifyMinedReceipt(c.bundle,transactionHash,adapter);
 equal(result.tx.from,call.from,'transaction caller');equal(result.tx.to,call.to,'transaction target');
 equal(result.tx.data,call.data,'transaction calldata');equal(result.tx.value,call.value,'transaction value');
 return result;
}
 ...
 */
export async function verifyMinedReceipt(b:VerifiedBundle,transactionHash:Hex,adapter:ProtocolReceiptAdapter){
 requireBundle(b);const result=await verifyMinedTransaction(b.policy.chainId,b.block.number,transactionHash,adapter);
...
export async function verifyMinedTransaction(chainId:bigint,notBeforeBlock:bigint,transactionHash:Hex,adapter:ProtocolReceiptAdapter){
 ...
 equal(tx.chainId,chainId,'transaction chain');equal(tx.hash,txHash,'transaction hash');
 equal(hash(field(receipt,'transactionHash')),txHash,'receipt hash');equal(uint(field(receipt,'status'),8),1n,'successful receipt');
 const height=uint(field(receipt,'blockNumber')),blockHash=hash(field(receipt,'blockHash'),true);
 if(height<notBeforeBlock)fail('Receipt predates authentication');
 const at=block(await adapter.readBlock(height));equal(at.hash,blockHash,'receipt block');equal(at.number,height,'receipt height');
 equal(tx.blockHash,blockHash,'mined transaction block');equal(tx.blockNumber,height,'mined transaction height');
 const logs=array(field(receipt,'logs'),4096).map(l=>{
  const removed=optional(l,'removed');if(removed!==undefined&&bool(removed))fail('Removed receipt log');
...
export async function decryptProtocolResult(result:VerifiedConfidentialResult,adapter:CotiDecryptionAdapter):Promise<bigint>{
 if(!privateResults.has(result))fail('Unverified confidential result');
